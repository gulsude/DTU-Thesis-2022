...
def attn_HLA_length_aatype_position_num(data, attn_data, n_heads, hla, corr = False, label = None, length = 9, show_num = False):
    data_correct = data
    aatype_position = dict()
    
    if corr:
        
        data_correct['bin_'] = [round(x) for x in list(data['pred_'])]
        if hla == None: 
            if length == None:
                
                length_index = np.array(data_correct[data_correct.label == label][data_correct['target_']==data_correct['bin_']].index)
            else:
                length_index = np.array(data[data.label == label][data.length ==length][data_correct['target_']==data_correct['bin_']].index)
...
def attn_HLA_length_aatype_position_pd(HLA_length_aatype_position, df_size, length = 9, n_heads = 3, softmax = True, unsoftmax = True):
    ...
    if length == None:
        HLA_length_aatype_position_pd = np.zeros((20, 14))
    else:
